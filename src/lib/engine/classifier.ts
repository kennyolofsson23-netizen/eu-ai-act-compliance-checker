...
const PROHIBITED_PRACTICES_LABELS: Record<string, string> = {
  ...
  vulnerability:
    "exploitation of vulnerabilities of specific groups (Article 5(1)(b))",
  ...
  emotion_workplace:
    "emotion recognition in workplace or educational settings (Article 5(1)(f))",
...
function classifyHighRisk(opts: {
  role: UserRole;
  articles: string[];
  reasoning: string;
  domain?: string;
...
function classifyLimitedRisk(
  answers: AssessmentAnswers,
  role: UserRole,
): ClassificationResult {
  const reasons: string[] = [];
  if (answers.interactsWithPeople)
    reasons.push("directly interacts with natural persons");
  if (answers.generatesSyntheticContent)
    reasons.push("generates or manipulates media content");
...
function checkHighRiskByDomainAndFunction(
  answers: AssessmentAnswers,
  role: UserRole,
): ClassificationResult | null {
  const domain = answers.domain ?? "other";
  const isHighRiskDomain = HIGH_RISK_DOMAINS.has(domain);
  const isHighRiskFunction = HIGH_RISK_FUNCTIONS.has(
...
export function classify(answers: AssessmentAnswers): ClassificationResult {
  if (answers.isAiSystem === false) {
    return {
      ...
      obligations: [],
      reasoning:
        "Your system does not meet the definition of an AI system under Article 3(1) of the EU AI Act and is therefore out of scope.",
    };
  }
  ...
  if (answers.isSafetyComponent === true) {
    return classifyHighRisk({
      ...
      reasoning:
        "Your AI system is a safety component of a product regulated under EU harmonised legislation, making it high-risk under Article 6(1) of the EU AI Act.",
  ...
  return {
    ...
    reasoning:
      "Your AI system does not fall into any of the prohibited, high-risk, or limited-risk categories. No mandatory obligations apply, though voluntary adherence to codes of conduct is encouraged under Article 95.",
  };
}
