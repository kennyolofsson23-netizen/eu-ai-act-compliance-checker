...
const ASSESSMENT_QUESTIONS = [
  ...
  },
  {
    ...
    type: 'select',
    options: ['General public', 'Specific organizations', 'Government agencies', 'Law enforcement', 'Critical infrastructure operators']
  },
  {
    id: 'data_types',
    question: 'What types of data does your system process?',
    type: 'checkbox',
    options: ['Biometric data', 'Personal identifiers', 'Financial data', 'Health information', 'Criminal records', 'Location data']
  },
  {
    id: 'decisions',
    question: 'Does your system make autonomous decisions affecting individuals?',
    type: 'select',
    options: ['Yes, legally binding decisions', 'Yes, recommendations', 'No, informational only']
  },
  {
    id: 'vulnerable_groups',
    question: 'Does your system target vulnerable populations?',
    type: 'select',
    options: ['Children', 'Elderly', 'People with disabilities', 'No specific vulnerable groups']
  },
  {
    id: 'transparency',
    question: 'How transparent is your system to users?',
    type: 'select',
    options: ['Users know they\'re interacting with AI', 'Partially transparent', 'Black box', 'Not applicable']
  },
  {
    id: 'human_oversight',
    question: 'Is there human oversight of AI decisions?',
    type: 'select',
    options: ['Always required', 'For critical decisions only', 'Minimal oversight', 'None']
  },
  {
    id: 'model_type',
    question: 'What type of AI model does your system use?',
    type: 'select',
    options: ['Large Language Model (LLM)', 'Computer Vision', 'Predictive Analytics', 'Recommendation System', 'Robotic Process Automation', 'Other']
  },
  {
    id: 'training_data',
    question: 'How much training data does your model use?',
    type: 'select',
    options: ['< 1 million samples', '1-100 million samples', '100 million - 1 billion samples', '> 1 billion samples']
  },
  {
    id: 'compliance_measures',
    question: 'What compliance measures do you currently have?',
    type: 'checkbox',
    options: ['Privacy by design', 'Impact assessments', 'Testing for bias', 'User documentation', 'Data protection measures', 'None']
  },
  {
    id: 'incidents',
    question: 'Have there been any negative incidents or complaints?',
    type: 'select',
    options: ['Multiple incidents', 'Some incidents', 'No known incidents', 'Unknown']
  },
  {
...
export default function ComplianceCheckerForm() {
  ...
  const submitForm = async () => {
    ...
    try {
      ...
      const mockResult: RiskResult = {
        ...
        description: 'Your AI system falls into the High-Risk category under the EU AI Act. This means you must implement substantial compliance measures.'
      }
  ...
  if (result) {
    return <ResultScreen result={result} onReset={() => {
      setResult(null)
                  ...
                  {question.options?.map((option) => (
                    ...
                    >
                      <input
                        ...
                        onChange={(e) => {
                          const current = Array.isArray(formData[question.id]) ? formData[question.id] : []
                          if (e.target.checked) {
...
function ResultScreen({ result, onReset }: { result: RiskResult; onReset: () => void }) {
  const riskColors = {
    ...
    <section className="py-12 md:py-20 bg-white">
      <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-8">
          <h1 className="text-4xl font-bold mb-4">Your Compliance Assessment</h1>
          <p className="text-lg text-slate-600">Based on your responses</p>
        </div>

        <div className={`rounded-lg border-2 p-8 mb-8 ${riskColors[result.level]}`}>
          <div className="text-center">
            <div className="text-5xl font-bold mb-4 capitalize">{result.level} Risk</div>
            <div className="text-lg font-medium">{result.percentage}% Compliance Effort Required</div>
