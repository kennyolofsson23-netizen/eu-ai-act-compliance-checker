...
export default function QuestionnaireShell() {
  ...
  const { state, answerQuestion, goBack, progress, totalQuestions } = useQuestionnaire()

  useEffect(() => {
    if (!state.isComplete || !state.result) return
    try {
      sessionStorage.setItem('eu_ai_result', JSON.stringify({
        result: state.result,
  ...
  const previousAnswer = state.answers[answerKey as keyof AssessmentAnswers] as string | string[] | undefined

  return (
    <div className="w-full max-w-2xl mx-auto">
      <ProgressBar current={currentStep} total={totalQuestions} percentage={progress} />
      ...
      <div className="mt-4 flex items-center justify-between">
        ...
        <p className="text-xs text-slate-400">Progress is saved automatically</p>
      </div>
    </div>
  )
}
